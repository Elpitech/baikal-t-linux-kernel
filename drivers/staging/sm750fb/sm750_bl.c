...
static void sm750_set_bl(unsigned char pwm, unsigned int level)
{
	...
	ulReg &= ~PWM_COUNTER_HIGH_MASK;
	ulReg |= (level << PWM_COUNTER_HIGH_SHIFT) & PWM_COUNTER_HIGH_MASK;

	value = ((MHZ(FREQUENCY)/10) * (BACKLIGHT_MAX - level)) >> 15;
	ulReg &= ~PWM_COUNTER_LOW_MASK;
	ulReg |= (level << PWM_COUNTER_LOW_SHIFT) & PWM_COUNTER_LOW_MASK;

	ulReg &= ~PWM_STATUS_MASK;
	ulReg |= (PWM_STATUS_INTERRUPT_PENDING | PWM_STATUS_ENABLE);
...
static void sm750_bl_init_pwm(struct sm750_dev *sm750_dev)
{
	...
	/* Enable PWM on the selected GPIO */
	...
	ulReg = PEEK32(base) & ~PWM_STATUS_MASK;
	ulReg |= PWM_STATUS_ENABLE;
	POKE32(base, ulReg);
}
