...
static void _crtc_init(struct drm_device *drm) {
    ...
    ret = drm_universal_plane_init(drm, plane, 0, &_plane_funcs, _formats, ARRAY_SIZE(_formats), DRM_PLANE_TYPE_PRIMARY, NULL);
    if (ret)
	return;
    drm_plane_helper_add(plane, &_plane_helper_funcs);

    ret = drm_crtc_init_with_planes(drm, crtc, plane, NULL, &_crtc_funcs, NULL);
    if (ret < 0) {
	drm_crtc_cleanup(crtc);
	return;
    }
...
static struct drm_encoder *_encoder_init(struct drm_device *drm) {
    ...
    drm_encoder_init(drm, encoder, &_encoder_funcs, DRM_MODE_ENCODER_DAC, NULL); //TODO: add extra params "char *name"
